...
class Viewer:
    ...
    def draw_trajectory(self, trajectory_list, agent_list):
        for i in range(len(trajectory_list)):
            for t in trajectory_list[i]:
                pygame.draw.circle(
                    self.background, COLORS[agent_list[i].color], t, 2, 1
                )
    ...
    def draw_energy_bar(self, agent_list):
        ...
        for agent_idx in range(len(agent_list)):
            remaining_energy = (
                agent_list[agent_idx].energy / agent_list[agent_idx].energy_cap
            )
            start_pos = [coord[agent_idx], 100]
            end_pos = [coord[agent_idx] + 50 * remaining_energy, 100]
            pygame.draw.line(
                self.background,
                color=COLORS[agent_list[agent_idx].color],
                start_pos=start_pos,
                end_pos=end_pos,
                width=5,
            )
    ...
    def draw_view(self, obs, agent_list):  # obs: [2, 100, 100] list
        ...
        for agent_idx in range(len(obs)):
            ...
            pygame.draw.circle(
                self.background,
                COLORS[agent_list[agent_idx].color],
                [coord[agent_idx] + 10, 55 + agent_list[agent_idx].r],
                agent_list[agent_idx].r,
                width=0,
            )
            pygame.draw.circle(
                self.background,
            ...
            pygame.draw.lines(
                ...
                points=[
                    [566 + 70 * agent_idx, 5],
                    [566 + 70 * agent_idx, 55],
                    [566 + 50 + 70 * agent_idx, 55],
                    [566 + 50 + 70 * agent_idx, 5],
                ],
                closed=True,
                color=COLORS[agent_list[agent_idx].color],
                width=2,
