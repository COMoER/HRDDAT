...
from olympics.func_utils import *
...
def line_intersect(
        line1, line2
):  # [[x1,y1], [x2,y2]], https://stackoverflow.com/questions/563198/how-do-you-detect-where-two-line-segments-intersect
...
class OlympicsBase(object):
    ...
    def _circle_collision_response(self, coord1, coord2, v1, v2, m1, m2):
        ...
        return [u1_x * self.restitution, u1_y * self.restitution], [
            u2_x * self.restitution,
            u2_y * self.restitution,
        ]
    ...
    def wall_response(self, target_idx, col_target, pos, v, r, t):
        ...
        col_pos, col_v = object.collision_response(
            pos, v, r, col_target, t, self.restitution
        )
    ...
    def stepPhysics(self, actions_list, step=None, is_test=False):
        ...
        if is_test:
            return temp_pos_container,temp_v_container
        ...
        for agent_idx, agent in enumerate(self.agent_list):
            ...
            for c in self.map["objects"]:
                if (c.type == "wall") or (c.type == "cross"):
                    distance = abs(
                        get_distance(c.cur_pos, vec_oc_, c.length, pixel=False)
                    )
                        ...
                        if obs_map[i][j] > 0:
                            ...
                            if (component.type == "wall") or (
                                ...
                                if check_radian(start_radian, end_radian, angle):
                                    ...
                                    if (distance <= radius + v_clear) and (
                                            distance >= radius - v_clear
                                    ):
                                        obs_map[i][j] = COLOR_TO_IDX[component.color]
    ...
    def get_map_grid(self, resolution,color=False):
        """resolution:[H,W]"""
        if color:
            obs_map = np.zeros((*resolution,3), np.uint8)
        else:
            obs_map = np.zeros((*resolution, 1),int)
        assert resolution[0] == resolution[1]
        h, w = 700, 700
        r = h / resolution[0]
        for component in self.map["objects"]:
            for i in range(resolution[0]):
                y = i * r
                for j in range(resolution[1]):
                    x = j * r
                    point = (x,y)
                    if np.sum(obs_map[i,j]) > 0:
                        continue
                    else:
                        if component.type == "wall" or component.type == "cross":
                            distance = abs(get_distance(component.init_pos, point, component.length, pixel=True))
                            if distance <= r:  # 距离小于等于1个像素点长度
                                if color:
                                    if component.color == 'red':
                                        obs_map[i, j] = np.array([0, 0, 255])
                                    if component.color == 'black':
                                        obs_map[i, j] = np.array([255, 255, 255])
                                    if component.color == 'grey':
                                        obs_map[i, j] = np.array([100, 100, 100])
                                else:
                                    if component.color == 'red':
                                        obs_map[i,j] = 2
                                    if component.color == 'black':
                        ...
                        #         obs_map[i][j] = COLOR_TO_IDX[component.color]
                            ...
                            angle = math.atan2(y_2center, x_2center)
                            start_radian, end_radian = (
                                component.start_radian,
                                component.end_radian,
                            )
                            if check_radian(start_radian, end_radian, angle):
                                vec = (x - component.center[0],y - component.center[1])
                                distance = distance_2points(vec)
                                if distance <= radius + r and distance >= radius - r:
                                    if color:
                                        obs_map[i,j] = np.array([255,255,255])
                                    else:
                                        obs_map[i,j] = 1
